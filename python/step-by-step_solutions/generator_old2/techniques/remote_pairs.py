...
def find_remote_pairs(options, chars, show_logs=False):
    ...
    for (i1, i2) in itertools.product(range(size), repeat=2):
        ...
        if len(options_for_cell) == 2:
            ...
            idxs = get_idxs_in_dimensions(dims, i1, i2)
            ...
            for potential_pair, potential_endpoints in potential_pairs_and_endpoints.items():
                if len(potential_endpoints) >= 2:
                    ...
                    potential_endpoint_combinations = list(itertools.combinations(potential_endpoints, 2))
                    for endpoint_start, endpoint_end in potential_endpoint_combinations:
                        if show_logs:
                            print(f"Try starting at {tuple(e + 1 for e in endpoint_start)} and ending at {tuple(e + 1 for e in endpoint_end)}")

                        valid_chains = search_valid_chains(
                            dims, options, potential_pair, endpoint_start, endpoint_end, show_logs
                        )
...
def search_valid_chains(dims, options, potential_pair, endpoint_start, endpoint_end, show_logs):
    ...
    def follow_chain(chain):
        ...
        idxs_in_dimensions = get_idxs_in_dimensions(dims, *current_point)
