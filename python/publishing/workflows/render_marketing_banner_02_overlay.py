...
DEFAULT_CONFIG = Path(
    ...
    "BK-CL9-DW-B01.marketing.phase_c2_banner_02_overlay.json"
)
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if args.dry_run:
        ...
        _write_json(reports_dir / "phase_c2_banner_02_overlay_dry_run_report.json", report)
    ...
    _write_json(reports_dir / "phase_c2_banner_02_overlay_report.json", report)
    print(json.dumps(report["outputs"], ensure_ascii=False, indent=2))
    print(f"[OK] Phase C2 Banner 2 overlay written to: {out_path}")
    return report


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render Banner 2 text overlay onto a locked background.")
    parser.add_argument("--config", default=str(DEFAULT_CONFIG), help="Phase C2 Banner 2 overlay config JSON.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        print(f"[ERROR] Banner 2 overlay render failed: {exc}", file=sys.stderr)
        return 1
