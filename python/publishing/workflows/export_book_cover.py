...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export a publication package cover to a printable full-wrap PDF."
    )
    parser.add_argument(
        "--publication-dir",
        required=True,
    ...
    parser.add_argument(
        "--output-pdf",
        required=True,
        help="Path to the output cover PDF file.",
    )
...
def main() -> int:
    ...
    _log("=" * 72)
    _log("export_book_cover.py starting")
    _log("=" * 72)
    ...
    try:
        written = export_book_cover_pdf(
            publication_dir=publication_dir,
            output_pdf_path=output_pdf,
        )
    except Exception as exc:
        _log(f"ERROR while exporting cover PDF: {exc}")
        return 1

    _log(f"Cover PDF written successfully: {written}")
    _log("=" * 72)
    _log("export_book_cover.py completed successfully")
    _log("=" * 72)
    return 0
