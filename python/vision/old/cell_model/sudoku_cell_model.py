...
def build_backbone(input_shape=(64, 64, 1)):
    base = keras.applications.MobileNetV3Small(
        input_shape=(64, 64, 3), include_top=False, alpha=0.5, minimalistic=True, pooling="avg"
    )
...
def masked_losses(y_true, y_pred):
    ...
    mask_digit = tf.cast(
        tf.reduce_any(tf.one_hot([1, 2], depth=4) == tf.expand_dims(type_idx, -1), axis=-1),
        tf.float32,
    )
