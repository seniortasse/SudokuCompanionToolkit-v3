# opencv_rectify.py — robust Sudoku rectifier with adaptive lattice + corner-by-intersection
# + NEW: refit-and-extend selected lines that don't span the ROI to fix "short horizontals/verticals"
...
def build_oriented_masks(gray: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # Long bars to isolate near-horizontal / near-vertical ink
    ...
    # Heal small gaps along each axis (glare / low ink)
...
# ───────── collinear grouping (repairs broken lines) ─────────
def _group_collinear_components(mask: np.ndarray, axis: str, approx_cell: float):
    ...
    tol = 0.18*approx_cell  # well below cell size
    ...
    for g in groups:
        ...
        for k, (x0, y0, x1, y1), c in g:
            gm[lab == k] = 255  # union of pieces from original labeling
...
def _select_8_lines(mask: np.ndarray, axis: str, approx_cell: float, other_mask: np.ndarray) -> List[np.ndarray]:
    ...
    for min_span_frac, thick_frac in span_thick:
        ...
        # Fallback on this span/thickness: coverage + spacing only
    ...
    # Last resort: most central with coverage
    if len(merged) >= 8:
        center = H/2.0 if axis=='h' else W/2.0
        fallback_band = gates[-1][1]
        filt = [(abs(center - c), m) for (m,b,c) in merged if _central_coverage(m, axis, fallback_band) >= min_cov]
        if len(filt) >= 8:
...
def _refine_selected_masks(masks: List[np.ndarray], axis: str, W: int, H: int, approx_cell: float) -> List[np.ndarray]:
    ...
    for m in masks:
        ...
        if axis == 'h':
            span = xs.max() - xs.min()
            need_refit = span < 0.90*W  # "short horizontal"
        else:
            span = ys.max() - ys.min()
            need_refit = span < 0.90*H  # "short vertical"
    ...
    for i, hm in enumerate(h_masks):
        ...
        for j, vm in enumerate(v_d):
            ...
            if xs.size:
                ...
                # Fallback: pick nearest strong overlap by distance transform
...
def _complete_lattice_adaptive(P8: np.ndarray) -> np.ndarray:
    ...
    # corners via intersections of local border fits
    ...
    def _first3(arr):  return arr[:3] if len(arr) >= 3 else arr
    def _last3(arr):   return arr[-3:] if len(arr) >= 3 else arr
...
def _warp_cell(gray_img: np.ndarray, P: np.ndarray, r: int, c: int, out: int = 64, shrink: float = 0.14) -> np.ndarray:
    ...
    M = cv2.getPerspectiveTransform(
        np.array([q[0], q[1], q[2], q[3]], np.float32),
...
def process(image_path: str, out_dir: str, tile_size: int = 64) -> dict[str, Any]:
    ...
    # S1 debug for chosen angle (pre-refine)
    ...
    # NEW: refit any selected lines that do not span the ROI (fix "short horizontals/verticals")
