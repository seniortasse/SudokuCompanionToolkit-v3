...
out/
  ...
  points_10x10.json  – 100 lattice points in ROI space
  rectify_debug/
...
def _group_collinear_components(mask: np.ndarray, axis: str, approx_cell: float):
    ...
    tol = 0.24*approx_cell  # slightly wider to merge soft/bowed fragments
...
# ──────────────────────────────────────────────────────────────────────────────
# 8) SELECT EXACTLY 8 INTERNAL LINES (strict path)
# ──────────────────────────────────────────────────────────────────────────────
    ...
    for min_span_frac, thick_frac in span_thick:
        ...
        for center_margin_frac, band_frac, ignore_frac, crossings_min in gates:
            ...
            for (cmask, b, coord) in base:
                if axis == 'h':
                    if coord < center_margin or (H - coord) < center_margin:
                        continue
                else:
                    if coord < center_margin or (W - coord) < center_margin:
                        continue
                if _central_coverage(cmask, axis, band_frac) < min_cov:
                    continue
...
def _complete_lattice_adaptive(P8: np.ndarray) -> np.ndarray:
    ...
    def _fit_h(points: np.ndarray) -> Tuple[float, float]:
    ...
    def _fit_v(points: np.ndarray) -> Tuple[float, float]:
        x = points[:, 0].astype(np.float64); y = points[:, 1].astype(np.float64)
        if len(points) < 2:
            return 0.0, float(np.median(x) if len(x) else 0.0)
...
def process(image_path: str,
    ...
    """
    Run the rectification pipeline.
    ...
    # ── 1) FAST ANGLE SWEEP (strict path) ──
    ...
    if best is None and robust:
        # ── Robust fallback: pre-crop + center-biased selection ──
        ...
        # Normal strict path from best angle
    ...
    # ── Common continuation (both paths) ──
    Hroi2, Wroi2 = gray_roi.shape[:2]
    approx_cell = min(Hroi2, Wroi2)/9.0
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="Sudoku photo → 81 clean tiles")
    g = p.add_mutually_exclusive_group(required=True)
    g.add_argument("--image", help="Path to one image")
    g.add_argument("--batch", help="Folder of images (jpg/png) to process")

    p.add_argument("--out", required=True, help="Output folder (root)")
    p.add_argument("--tile-size", type=int, default=64)
    p.add_argument("--precompress", action="store_true")
    p.add_argument("--target-kb", type=int, default=120)
    p.add_argument("--max-side", type=int, default=1600, help="Work max long side")
    p.add_argument("--min-jpeg-quality", type=int, default=45)
    p.add_argument("--shrink", type=float, default=0.14, help="Cell inset 0..0.3")
    p.add_argument("--robust", action="store_true", help="Try robust fallback (pre-crop + lenient rail selection) if strict pass fails")
    ...
    def run_one(img_path: str):
        ...
        print(f"[rectify] {img_path} → {out_dir}")
        print(process(
            image_path=img_path,
            out_dir=out_dir,
            ...
            work_max_long_side=args.max_side,
    ...
    if args.image:
        ...
        exts = ("*.jpg", "*.jpeg", "*.png")
        imgs = [p for e in exts for p in glob.glob(os.path.join(args.batch, "**", e), recursive=True)]
        for img in imgs:
            run_one(img)
