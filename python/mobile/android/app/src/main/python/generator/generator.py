...
from generator.verification import check_is_unique_solution
...
def can_remove_idx(instance, idxs, solution, only_accept_human_solvable_instances, allowed_techniques):
    ...
    _counts_techniques, _logs = None, None
    ...
    if chars_present != instance.chars:
        ...
        return result, (_counts_techniques, _logs)
    ...
    if only_accept_human_solvable_instances:
        ...
        solved_instance, (_counts_techniques, _logs) = solve_using_human_techniques(
...
def generate_instance(solution, target_non_empty_cells=0, allowed_techniques=TECHNIQUES, max_uses_techniques={},
    ...
    # These logs were not used anywhere before using them to generate reversed step-by-step user logs, and was likely
    #  fortunately introduced for testing purposes
    logs = {}
    logs["steps"] = []
    ...
    if (_remove_idxs := options.get("remove-idxs")) is not None:
        ...
        for idx in _remove_idxs:
            ...
            result, _ = can_remove_idx(instance, [idx], solution, only_accept_human_solvable_instances, allowed_techniques)
            if result == "yes":
                instance[idx[0]][idx[1]] = EMPTY_CHAR
            else:
    ...
    while len(idxs_to_check) > 0 and count_non_empty_cells(instance) > target_non_empty_cells:
        ...
        # Used for logs only
        instance_for_step = copy_instance(instance)
        ...
        if sym is not None:
            if sym == "center":
                idx_sym = (size - 1 - idx_to_remove[0], size - 1 - idx_to_remove[1])
            elif sym == "horizontal":
                idx_sym = (idx_to_remove[0], size - 1 - idx_to_remove[1])
            elif sym == "vertical":
                idx_sym = (size - 1 - idx_to_remove[0], idx_to_remove[1])
            elif sym == "diagonal-1":
                idx_sym = (idx_to_remove[1], idx_to_remove[0])
            elif sym == "diagonal-2":
                idx_sym = (size - 1 - idx_to_remove[1], size - 1 - idx_to_remove[0])
            else:
            ...
            if idx_sym != idx_to_remove:
                # Note: To avoid crashing when subwords/diagonal have been removed first, check whether the value is
                #  still present
                if idx_sym in idxs_to_check:
                    idxs_to_remove.append(idx_sym)

        print(f"Try to remove {[solution[idx[0]][idx[1]] for idx in idxs_to_remove]} at {idxs_to_remove} ({len(idxs_to_check)} more remaining)")

        for idx in idxs_to_remove:
            idxs_to_check.remove(idx)

        result, (_counts_techniques, _logs) = can_remove_idx(instance, idxs_to_remove, solution, only_accept_human_solvable_instances, allowed_techniques)
        if _counts_techniques:
            print(_counts_techniques)
        if result == "yes":
            for idx in idxs_to_remove:
                instance[idx[0]][idx[1]] = EMPTY_CHAR
            print(" -> successful")
        ...
        # Update logs
        # Note that there is no break from this loop, unless an exception occurs, as all exit conditions are checked in
        #  the while-statement, so the logs for the last step will be included
        logs_step = (instance_for_step, idxs_to_remove, result, _counts_techniques)
    ...
    assert check_is_unique_solution(instance, solution, preprocess=False)
