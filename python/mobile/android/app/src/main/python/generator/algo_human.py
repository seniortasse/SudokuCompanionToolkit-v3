...
from generator.model import EMPTY_CHAR, count_empty_cells, copy_instance, Instance, DIMENSIONS
from generator.techniques.singles import find_singles_1, find_singles_2, find_singles_3, find_naked_singles_2, find_naked_singles_3
...
from generator.techniques.ab_chains import find_ab_chains
...
from generator.techniques.leftovers import find_leftovers_1, find_leftovers_2, find_leftovers_3, find_leftovers_4, \
    find_leftovers_5, find_leftovers_6, find_leftovers_7, find_leftovers_8, find_leftovers_9
...
def _trace_copy_options(options):
    """
    Trace-only snapshot of the current options grid.

    We intentionally keep this lightweight and deterministic:
    each cell becomes a sorted list of candidate chars.
    """
    if options is None:
        return None
    ...
    for row in options:
        out_row = []
        for cell_options in row:
            try:
                out_row.append(sorted(list(cell_options)))
            except Exception:
                out_row.append([])
        out.append(out_row)
    return out
...
def _trace_hit_payload(hit):
    """
    hit shapes currently seen in this engine:
      ((i1, i2), char)
      ((i1, i2), char, dim)
      ((i1, i2), char, dim, details)
    """
    try:
        idx = hit[0]
    except Exception:
...
def _trace_removed_chars_payload(removed_chars):
    out = []
    for item in (removed_chars or []):
        try:
            idx, char = item
            out.append({
                "cell": _trace_rc_to_payload(idx),
...
def _trace_application_payload(detail):
    """
    detail shape after apply_technique:
      ((technique_id, name_application), application_details, removed_chars_updated)
    """
    try:
        (technique_id, name_application), application_details, removed_chars_updated = detail
        return {
            "technique_id": technique_id,
            "name_application": name_application,
            "application_details": application_details,
            "removed_chars_effective": _trace_removed_chars_payload(removed_chars_updated),
            "effective_removed_count": len(removed_chars_updated or []),
        }
    except Exception:
        return {
            "technique_id": None,
            "name_application": "malformed_cleanup_detail",
            "application_details": detail,
            "removed_chars_effective": [],
            "effective_removed_count": 0,
        }


def _trace_cleanup_batch_payload(
    *,
    technique_latest,
    technique_cleanup,
    iteration,
    ...
    options_after,
    details_updated,
    number_removed_values,
    hits,
):
    return {
        "technique_latest": technique_latest,
        "technique_cleanup": technique_cleanup,
        "iteration": iteration,
        "options_before": _trace_copy_options(options_before),
        "options_after": _trace_copy_options(options_after),
        "applications_effective": [
            _trace_application_payload(detail)
            for detail in (details_updated or [])
        ],
        "number_removed_values": int(number_removed_values or 0),
        "new_values_found": [
            _trace_hit_payload(hit)
            for hit in (hits or [])
        ],
        "hit_found": len(hits or []) > 0,
        "hit_count": len(hits or []),
    }


def _trace_selected_hit_payload(hits):
    if not hits:
        return None
    return _trace_hit_payload(hits[0])


# Note: This also defines the order in which techniques are applied
# TODO Make sure this corresponds with the weights!
TECHNIQUES = [
...
TECHNIQUES_REQUIRING_STANDARD_BOXES_LAYOUT = [
    "boxed-doubles", "boxed-triplets", "boxed-quads", "boxed-wings", "boxed-rays"
]
assert not set(TECHNIQUES_REQUIRING_STANDARD_BOXES_LAYOUT).difference(TECHNIQUES)


fncs = {
    "singles-1": find_singles_1,
    "singles-2": find_singles_2,
    "singles-3": find_singles_3,
    ...
    "leftovers-1": find_leftovers_1,
    "leftovers-2": find_leftovers_2,
    "leftovers-3": find_leftovers_3,
    "leftovers-4": find_leftovers_4,
    "leftovers-5": find_leftovers_5,
    "leftovers-6": find_leftovers_6,
    "leftovers-7": find_leftovers_7,
    "leftovers-8": find_leftovers_8,
    "leftovers-9": find_leftovers_9,
...
def solve_using_human_techniques(instance, use_techniques=TECHNIQUES,
    ...
    logs = {}
    logs["steps"] = []
    # Series 1 scaffolding:
    # parallel trace channel; does not change the external logged-step contract yet
    logs["step_traces"] = []
    ...
    while not is_filled:
        ...
        # Series 2 causal placeholders.
        # Safe even if Series 1 trace scaffolding has already been added.
        log_causal_resolution = None
        log_causal_applications = []

        # Series 1 scaffolding:
        # per-step engine trace, kept in parallel to the existing tuple log
        log_step_trace = {
            "step_index": step,
            "grid_before": copy_instance(solved_instance),
            "base_techniques_attempted": [],
            "advanced_batches": [],
            "selected_hit": None,
            "selected_hit_source": None,
            "selected_hit_is_cleanup_issue": False,
        }
        ...
        for name in TECHNIQUES:
            if name in BASE_TECHNIQUES and name in use_techniques:
                ...
                log_step_trace["base_techniques_attempted"].append({
                    "technique_id": name,
                    "hit_count": len(hits),
                    "hits": [_trace_hit_payload(hit) for hit in hits],
                })
                ...
                if len(hits) > 0:
                    ...
                    log_causal_resolution = {
                        "mode": "direct",
                        "causal_application_indices": [],
                        "reason": "base_technique_direct_hit",
                        "tested_combo_sizes": [],
                    }
                    log_causal_applications = []

                    log_step_trace["selected_hit"] = _trace_selected_hit_payload(hits)
                    log_step_trace["selected_hit_source"] = {
                        "phase": "base",
                        "technique_id": name,
                    }
                    log_step_trace["selected_hit_is_cleanup_issue"] = False
                    log_step_trace["causal_resolution"] = log_causal_resolution
                    log_step_trace["causal_applications"] = log_causal_applications
                    break
        ...
        if not could_use_technique and not use_cleanup_method:
            ...
            for name in TECHNIQUES:
                if name not in BASE_TECHNIQUES and name in use_techniques:
                    ...
                    time_start = time.time()
                    number_removed_values, hits, options, details = apply_technique(name, options, solved_instance, solution=magic_solution, show_logs=show_logs)
                    log_cleanup_steps.append((name, name, 0, options_before, details))
                    log_step_trace["advanced_batches"].append(
                        _trace_cleanup_batch_payload(
                            technique_latest=name,
                            technique_cleanup=name,
                            iteration=0,
                            options_before=options_before,
                            options_after=options,
                            details_updated=details,
                            number_removed_values=number_removed_values,
                            hits=hits,
                        )
                    )
                    time_end = time.time()
                    ...
                    if len(hits) > 0:
                        ...
                        log_causal_resolution = _resolve_causal_application_indices_for_selected_hit(
                            options_before=options_before,
                            details_updated=details,
                            selected_hit=hits[0],
                            chars=chars,
                        )
                        log_causal_applications = _trace_application_summary_for_indices(
                            details,
                            log_causal_resolution["causal_application_indices"],
                        )
        ...
        if not could_use_technique and use_cleanup_method:
            ...
            for technique_latest in try_techniques:
                ...
                time_start = time.time()
                number_removed_values, hits, options, details = apply_technique(technique_latest, options, solved_instance, solution=magic_solution, show_logs=show_logs)
                log_cleanup_steps.append((technique_latest, technique_latest, 0, options_before, details))
                log_step_trace["advanced_batches"].append(
                    _trace_cleanup_batch_payload(
                        technique_latest=technique_latest,
                        technique_cleanup=technique_latest,
                        iteration=0,
                        options_before=options_before,
                        options_after=options,
                        details_updated=details,
                ...
                if len(hits) > 0:
                    ...
                    while True:
                        ...
                        for technique_cleanup in included_techniques:
                            ...
                            time_start = time.time()
                            number_removed_values, hits, options, details = apply_technique(technique_cleanup, options, solved_instance, solution=magic_solution, show_logs=show_logs)
                            log_cleanup_steps.append((technique_latest, technique_cleanup, counter_subprocess, options_before, details))
                            log_step_trace["advanced_batches"].append(
                                _trace_cleanup_batch_payload(
                                    technique_latest=technique_latest,
                                    technique_cleanup=technique_cleanup,
                                    iteration=counter_subprocess,
                                    options_before=options_before,
                                    options_after=options,
                                    details_updated=details,
                                    number_removed_values=number_removed_values,
                            ...
                            if len(hits) > 0:
                                ...
                                log_step_trace["selected_hit"] = _trace_selected_hit_payload(hits)
                                log_step_trace["selected_hit_source"] = {
                                    "phase": "cleanup_subprocess",
                                    "technique_id": technique_latest,
                                    "technique_latest": technique_latest,
                                    "technique_cleanup": technique_cleanup,
                                    "iteration": counter_subprocess,
                                }
                                log_step_trace["selected_hit_is_cleanup_issue"] = technique_cleanup != technique_latest
                                log_step_trace["causal_resolution"] = log_causal_resolution
                                log_step_trace["causal_applications"] = log_causal_applications
                                break
        ...
        log_step_trace["grid_after"] = copy_instance(solved_instance)
        log_step_trace["selected_hit_technique_used"] = log_technique_used
        log_step_trace["selected_hit_cleanup_issue"] = log_is_cleanup_issue
        log_step_trace["selected_hit_count"] = len(log_new_values_found or [])
        log_step_trace["cleanup_batch_count"] = len(log_step_trace["advanced_batches"])
        log_step_trace["base_technique_count"] = len(log_step_trace["base_techniques_attempted"])
        log_step_trace["causal_resolution"] = log_causal_resolution
        log_step_trace["causal_applications"] = log_causal_applications
        ...
        logs["step_traces"].append(log_step_trace)
            ...
            try:
                for dim in DIMENSIONS:
                    for idx_dim in range(size):
                        vals_for_dim = solved_instance.get_values(dim, idx_dim)
                        for char in chars:
                            assert vals_for_dim.count(char) <= 1, f"Something wrong with the partial solution: {dim} {idx_dim + 1} has duplicate char '{char}'.."
    ...
    # Check: The number of empty cells of the successfully generated instance should match the techniques counted
    if is_filled:
...
def _trace_hit_identity(hit):
    """
    Extract the deterministic identity of a hit as ((i1, i2), char).
    Works for both raw engine hits and trace-derived hit tuples.
    """
    idx = hit[0]
    char = hit[1]
    return tuple(idx), char


def _trace_hit_matches_selected(hit, selected_hit):
    try:
        return _trace_hit_identity(hit) == _trace_hit_identity(selected_hit)
    except Exception:
        return False


def _trace_apply_removed_chars_to_options(options_snapshot, removed_chars):
    """
    Replay helper:
    apply a set of effective removals to a copied options snapshot.
    """
    options_replayed = copy_options(options_snapshot)

    for removed in (removed_chars or []):
        try:
            (i1, i2), char = removed
            try:
                options_replayed[i1][i2].remove(char)
            except KeyError:
                pass
        except Exception:
            pass

    return options_replayed


def _trace_replay_details_and_identify_hits(
    *,
    options_before,
    details_updated,
    detail_indices,
    ...
    """
    Replay only the specified effective applications from one batch, then
    recompute new values using the same identify_new_values() function the
    engine already uses.
    """
    options_replayed = copy_options(options_before)

    for idx in detail_indices:
        (_technique_meta, _application_details, removed_chars_updated) = details_updated[idx]
        for removed in (removed_chars_updated or []):
            try:
                (i1, i2), char = removed
                try:
                    options_replayed[i1][i2].remove(char)
    ...
    replay_hits = identify_new_values(options_replayed, chars, show_logs=False)
    replay_hits = [(*new_value, None) for new_value in replay_hits]
    replay_hits = _remove_duplicate_hits(replay_hits)

    return replay_hits


def _resolve_causal_application_indices_for_selected_hit(
    *,
    options_before,
    details_updated,
    selected_hit,
    ...
    """
    Determine which effective application(s) from a cleanup batch causally
    produce the selected hit.
    ...
    if not details_updated:
        return {
            "mode": "none",
            "causal_application_indices": [],
            "reason": "details_updated_empty",
            "tested_combo_sizes": [],
        }

    total = len(details_updated)
    tested_combo_sizes = []

    # 1) Single-application causality
    tested_combo_sizes.append(1)
    single_matches = []
    for idx in range(total):
        replay_hits = _trace_replay_details_and_identify_hits(
            options_before=options_before,
            details_updated=details_updated,
            detail_indices=[idx],
            chars=chars,
        )
        if any(_trace_hit_matches_selected(hit, selected_hit) for hit in replay_hits):
            single_matches.append(idx)

    if single_matches:
        return {
            "mode": "single",
            "causal_application_indices": [single_matches[0]],
            "reason": "single_application_forces_selected_hit",
            "tested_combo_sizes": tested_combo_sizes,
        }

    # 2) Multi-application causality (bounded)
    upper = min(max_combo_size, total)
    if total > 8:
        upper = min(2, upper)

    for combo_size in range(2, upper + 1):
        tested_combo_sizes.append(combo_size)
        for combo in itertools.combinations(range(total), combo_size):
            replay_hits = _trace_replay_details_and_identify_hits(
                options_before=options_before,
                details_updated=details_updated,
                detail_indices=list(combo),
                chars=chars,
            )
            if any(_trace_hit_matches_selected(hit, selected_hit) for hit in replay_hits):
                return {
                    "mode": "combo",
                    "causal_application_indices": list(combo),
                    "reason": f"{combo_size}_application_combo_forces_selected_hit",
                    "tested_combo_sizes": tested_combo_sizes,
                }
    ...
    return {
        "mode": "fallback",
        "causal_application_indices": [],
        "reason": "no_causal_subset_found",
        "tested_combo_sizes": tested_combo_sizes,
    }


def _trace_application_summary_for_indices(details_updated, indices):
    out = []
    for idx in indices or []:
        try:
            (technique_meta, application_details, removed_chars_updated) = details_updated[idx]
            technique_id, name_application = technique_meta
            out.append({
                "detail_index": idx,
                "technique_id": technique_id,
                "name_application": name_application,
                "application_details": application_details,
                "effective_removed_count": len(removed_chars_updated or []),
                "removed_chars_effective": removed_chars_updated,
            })
        except Exception:
            out.append({
                "detail_index": idx,
                "technique_id": None,
                "name_application": "malformed_detail",
...
# This is the core logic of how found applications are processed, and this should be designed properly
def apply_technique(name, options, solved_instance, solution=None, show_logs=False):

    fnc = fncs[name]

    chars = solved_instance.chars
    ...
    details_updated = []
    # TODO I did not know we collect all applications when applying a technique and remove all options at the same time,
    #  but only assign them to the application if they had not yet been removed by a previous application! There is room
    #  for improvement here; It would be better to always select an application, and apply only one, which could be the
    #  first one removing at least one option; The applications should be provided sorted by the technique, which will
    #  usually just be by row/col/box as they are already found, but some techniques require advanced sorting such as
    #  the chaining techniques, which are sorted by chain length; After the options for one application are removed,
    #  the search is continued with easier techniques; This is besides a cleaner implementation also much more
    #  realistic; It also tackles the long-standing improvement of iteratively applying a technique, but first trying
    #  all easier techniques again after one valid application has been found;
    for name_application, application_details, removed_chars in details:
        ...
        for (i1, i2), char in removed_chars:
            ...
            try:
                ...
                pass
            # Check: The option removed is not the value used in the solution
            # TODO At some point make sure that the solution is always passed
            if solution is not None:
                assert solution[i1][i2] != char, f"Removed char '{char}' at {(i1 + 1, i2 + 1)} invalid, as it is used in the solution!"
...
def parse_cleanup_logs(logs):
    ...
    for step_idx, step in enumerate(steps):
        if isinstance(step, (list, tuple)) and len(step) >= 6:
            steps_cleanup = step[5]
        else:
            steps_cleanup = []

        step_parsed = []
        for step_cleanup in (steps_cleanup or []):
            try:
            ...
            if len(details) > 0:
                for ((name_technique, name_application), _, removed_chars) in details:
                    _removed_chars = [
                        (tuple(map(lambda x: x + 1, cell_idx)), char)
                        for cell_idx, char in removed_chars
                    ]
                    if len(_removed_chars) > 0:
                        step_parsed.append(
                            (name_technique, name_application, removed_chars, options_before_application)
                        )
