...
def largest_quadrilateral(gray):
    ...
    thr = cv2.adaptiveThreshold(blur,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,31,5)
...
def remove_grid_lines(gray):
    ...
    inv = 255 - gray
    horiz = cv2.morphologyEx(inv, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_RECT,(25,1)))
...
def extract_cells(board_gray, outdir, center_margin=6):
    H,W = board_gray.shape[:2]; step = H//9
    out = []
    Path(outdir).mkdir(parents=True, exist_ok=True)
    for r in range(9):
        for c in range(9):
            y0=r*step; x0=c*step; y1=y0+step; x1=x0+step
            crop = board_gray[y0+center_margin:y1-center_margin, x0+center_margin:x1-center_margin]
            p = Path(outdir)/f"r{r+1}c{c+1}.png"
            cv2.imwrite(str(p), crop); out.append({"r":r+1,"c":c+1,"path":str(p)})
    return out
...
if __name__ == "__main__":
    import sys
    if len(sys.argv)<3:
        print("Usage: python opencv_rectify.py <image_path> <export_dir>"); sys.exit(1)
