...
def make_one_cell(idx, typ=None):
    if typ is None:
        typ = random.choices(
            [TYPE_BLANK, TYPE_PRINTED, TYPE_HAND, TYPE_NOTES], weights=[0.4, 0.25, 0.2, 0.15]
        )[0]
    digit = 0
    notes_vec = [0] * 9
    if typ == TYPE_BLANK:
    ...
    if pad > 0:
        img = ImageOps.expand(img, border=pad, fill=255).resize(
