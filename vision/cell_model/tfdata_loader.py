...
def _parse_jsonl(line):
    tf.io.decode_json_example(line) if False else None
    ...
    t_notes.set_shape((9,))
    return path, {
        "type": tf.cast(t_type, tf.float32),
...
def _augment(img):
    ...
    try:
        ...
        img = tfa.image.rotate(
            img, angles=angle, interpolation="BILINEAR", fill_mode="constant", fill_value=1.0
        )
    except Exception:
        pass
