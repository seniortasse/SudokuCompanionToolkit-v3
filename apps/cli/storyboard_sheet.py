...
import argparse, json, glob, re, math
from pathlib import Path
from PIL import Image, ImageDraw, ImageFont, ImageOps

PAPERS = {
    "letter": (2550, 3300),  # 8.5x11 @ 300dpi
...
def build_storyboard(overlay_paths, out_prefix, paper="letter", cols=2, margin=80, gutter=40, title=None, moves=None):
    W,H = PAPERS[paper]
    ...
    if rows==0:
        # nothing to draw
        page.save(out_prefix + ".png"); page.save(out_prefix + ".pdf")
...
def main() -> None:
    ...
    png, pdf = build_storyboard(overlays, args.out, paper=args.paper, cols=args.cols, title=args.title, moves=moves)
