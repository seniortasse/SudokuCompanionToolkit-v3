...
from types_sudoku import Candidates, Grid, Move
...
from .solver_core import apply_move as _apply_move
...
def sanity_check(original: Grid, current: Grid) -> dict:
    ...
    for r in range(1, 10):
        for c in range(1, 10):
            if original[r - 1][c - 1] != 0 and current[r - 1][c - 1] not in (
                0,
                original[r - 1][c - 1],
            ):
                issues.append(
                    {
                        "type": "given_overwritten",
                        "cell": f"r{r}c{c}",
                        "given": original[r - 1][c - 1],
                        "found": current[r - 1][c - 1],
                    }
                )
    ...
    for r in range(1, 10):
        ...
        if dups:
            ...
            issues.append(
                {"type": "duplicate", "unit": f"r{r}", "digits": sorted(list(dups)), "cells": cells}
            )
    # cols
    for c in range(1, 10):
        ...
        if dups:
            ...
            issues.append(
                {"type": "duplicate", "unit": f"c{c}", "digits": sorted(list(dups)), "cells": cells}
            )
    # boxes
    for b in range(1, 10):
        br = (b - 1) // 3
        ...
        if dups:
            ...
            issues.append(
                {"type": "duplicate", "unit": f"b{b}", "digits": sorted(list(dups)), "cells": bad}
            )
...
def _append_unique(moves: list[dict], new_moves: list[dict]):
    seen = set(
        (m.get("technique"), m.get("cell"), m.get("digit"), tuple(sorted(m.get("eliminate", []))))
        for m in moves
    )
    for m in new_moves:
        key = (
...
def next_moves(
    ...
    if max_difficulty in ("locked", "xwing", "advanced") and len(out_moves) < max_moves:
        # find eliminations
        elims = find_locked_candidates_pointing(cur, cands) + find_locked_candidates_claiming(
            cur, cands
        )
        # apply each elimination and test for new singles
        for e in elims:
            if len(out_moves) >= max_moves:
                break
