...
def unit_cells_row(r:int): return [(r,c) for c in range(1,10)]
def unit_cells_col(c:int): return [(r,c) for r in range(1,10)]
...
def find_hidden_singles(grid:Grid, candidates:Dict[str,list]):
    ...
    for r in range(1,10):
        ...
        for d, cells in pos_for_digit.items():
            if len(cells)==1:
                key = cells[0]; rr,cc = key_to_rc(key)
                moves.append({
...
def find_locked_candidates_pointing(grid:Grid, candidates:Dict[str,list]):
    ...
    for b in range(1,10):
        ...
        for d in range(1,10):
            locs = [rc_to_key(r,c) for (r,c) in cells if rc_to_key(r,c) in candidates and d in candidates[rc_to_key(r,c)]]
            if len(locs) < 2: 
                continue
            ...
            if len(rows)==1:
                ...
                if elim:
                    moves.append({
                        ...
                        "eliminate": elim,
                        "in_box": locs,
                        "explanation": {"why": f"In box {b}, digit {d}'s candidates lie only in row {r}. Eliminate {d} from row {r} outside this box."},
                        "highlights": {"box": f"b{b}", "row": f"r{r}", "cells": elim, "in_box": locs}
                    })
            ...
            if len(cols)==1:
                ...
                if elim:
                    moves.append({
                        ...
                        "eliminate": elim,
                        "in_box": locs,
                        "explanation": {"why": f"In box {b}, digit {d}'s candidates lie only in column {c}. Eliminate {d} from column {c} outside this box."},
                        "highlights": {"box": f"b{b}", "col": f"c{c}", "cells": elim, "in_box": locs}
                    })
    return moves
...
def find_locked_candidates_claiming(grid:Grid, candidates:Dict[str,list]):
    ...
    for r in range(1,10):
        for d in range(1,10):
            locs = [rc_to_key(r,c) for c in range(1,10) if rc_to_key(r,c) in candidates and d in candidates[rc_to_key(r,c)]]
            if len(locs) < 2:
                continue
            ...
            if len(boxes)==1:
                ...
                if elim:
                    moves.append({
                        ...
                        "eliminate": elim,
                        "in_line": locs,
                        "explanation": {"why": f"In row {r}, digit {d}'s candidates are confined to box {b}. Eliminate {d} from other cells in box {b}."},
                        "highlights": {"box": f"b{b}", "row": f"r{r}", "cells": elim, "in_line": locs}
                    })
    # cols
    for c in range(1,10):
        for d in range(1,10):
            locs = [rc_to_key(r,c) for r in range(1,10) if rc_to_key(r,c) in candidates and d in candidates[rc_to_key(r,c)]]
            if len(locs) < 2:
                continue
            boxes = sorted(set([which_box(*key_to_rc(k)) for k in locs]))
            if len(boxes)==1:
                ...
                if elim:
                    moves.append({
                        ...
                        "eliminate": elim,
                        "in_line": locs,
                        "explanation": {"why": f"In column {c}, digit {d}'s candidates are confined to box {b}. Eliminate {d} from other cells in box {b}."},
                        "highlights": {"box": f"b{b}", "col": f"c{c}", "cells": elim, "in_line": locs}
                    })
    return moves
