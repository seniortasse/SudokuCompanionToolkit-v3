...
# ============================================================================
# Typical Powershell CLI
# ============================================================================
"""
# Collect/normalize handwritten digits into a clean 96×96 bank + manifest
# - Reads EMNIST .mat files under datasets/digits/matlab (any depth)
# - Produces 96x96 white-background PNGs under datasets/digits96/<source>/<digit>/
# - Writes a JSONL manifest (one record per image): {"path","digit","source"}

python -m tools.collect_digits96 `
  --emnist-root "datasets/digits/matlab" `          # ROOT folder containing EMNIST .mat files.
                                                    # Your folder has: emnist-balanced, emnist-byclass,
                                                    # emnist-bymerge, emnist-digits, emnist-letters, emnist-mnist.
                                                    # The script recursively scans for *.mat and will try to parse
                                                    # any it finds. Only digit labels 0..9 are kept.
  --out-root   "datasets/digits96" `                # Where normalized 96x96 PNGs are saved.
                                                    # Output structure: datasets/digits96/<source>/<digit>/*.png
                                                    #   <source> will be "emnist" for EMNIST rows.
  --manifest   "datasets/digits96/digits96_manifest.jsonl" `  # JSONL file listing every saved image.
                                                    # Each line: {"path": "<posix path>", "digit": int, "source": "emnist"}
  --max-per-digit 0 `                               # Per-digit cap per source (0 = no cap).
                                                    # Set this to a positive number to limit class imbalance or keep
                                                    # the dataset small (e.g., 500 means up to 500 “3”s, 500 “7”s, etc).
  --overwrite                                       # If an output PNG already exists, recreate it.
                                                    # Useful when you change normalization; omit this to skip existing files.
"""
# ============================================================================